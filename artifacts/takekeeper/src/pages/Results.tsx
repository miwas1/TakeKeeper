...
export default function Results() {
  ...
  const current = data?.takes.find((take) => !take.isReference);
  return (
    <div className="space-y-6">
      ...
      <header><Badge variant="outline">ANALYSIS READY</Badge><h1 className="mt-3 text-3xl font-bold">Continuity results</h1><p className="mt-2 text-muted-foreground">{current ? `Take ${String(current.takeNumber).padStart(2, "0")} is saved and ready for comparison.` : "Upload a new take before running a comparison."}</p></header>
      <div className="grid gap-4 lg:grid-cols-[1fr_1fr_0.8fr]">
        ...
        <Card><CardHeader><CardTitle className="text-sm">Issues</CardTitle></CardHeader><CardContent className="py-8 text-center"><ScanSearch className="mx-auto mb-3 h-8 w-8 text-muted-foreground" /><p className="font-medium">No comparison has run yet.</p><p className="mt-2 text-sm text-muted-foreground">Gemini continuity analysis is reserved for Phase 3. No findings are fabricated here.</p></CardContent></Card>
      </div>
